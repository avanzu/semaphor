...
describe('Intent', () => {
    ...
    it('should be isomorp with a promise', async () => {
        expect.assertions(1)

        const goodIntent  = Intent.fromPromise(Promise.resolve('Bueno'))
        const goodPromise = goodIntent.toPromise()

        const badIntent   = Intent.fromPromise(Promise.reject('No bueno!'))
        const badPromise  = badIntent.toPromise()

        await expect(goodPromise).resolves.toEqual('Bueno')
        await expect(badPromise).rejects.toEqual('No bueno!')
    })
